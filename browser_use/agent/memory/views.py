...
class MemoryConfig(BaseModel):
	...
	# Embedder settings
	embedder_provider: Literal['openai', 'gemini', 'ollama', 'huggingface'] = 'openai'
	embedder_model: str = Field(min_length=2, default='bge-small-en-v1-5')
	...
	def llm_config_dict(self) -> dict[str, Any]:
		...
		return {'provider': 'openai', 'config': {'model': self.llm_instance}}
	...
	def vector_store_config_dict(self) -> dict[str, Any]:
		...
		return {
			...
			'config': {
				...
				'collection_name': f'mem0_{self.agent_id}',
				'normalize_L2': True,
