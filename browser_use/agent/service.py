...
class Agent(Generic[Context]):
	...
	async def step(self, step_info: Optional[AgentStepInfo] = None) -> None:
		...
		try:
			...
			if self.enable_memory and self.memory and self.state.n_steps % self.memory.config.memory_interval == 0:				self.memory.create_procedural_memory(self.state.n_steps)
